...
describe('pending-card script-validation verdict (issue #10861)', () => {
  ...
  it('shows the expandable findings warning in the expanded review panel', async () => {
    ...
    expect(screen.getByText(/Approval will be refused until its scripts are fixed/)).toBeInTheDocument()
  ...
  it('renders the refusal reason and findings after a failed approve', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('Approval refused: the bundled scripts failed validation — fix the scripts, then approve again.'),
      ).toBeInTheDocument(),
    )
  ...
  it('dismissing the candidate clears its refusal, so a re-staged slug starts clean', async () => {
    ...
    try {
      ...
      await waitFor(() =>
        expect(
          screen.queryByText('Approval refused: the bundled scripts failed validation — fix the scripts, then approve again.'),
  ...
  it('a failed dismiss surfaces a named error notice instead of failing silently', async () => {
    ...
    try {
      ...
      await waitFor(() =>
        expect(
          screen.getByText('Dismiss of “auto/evil-helper” failed (this candidate is no longer pending — approved or dismissed elsewhere).'),
        ).toBeInTheDocument(),
      )
      ...
      await waitFor(() =>
        expect(
          screen.queryByText('Dismiss of “auto/evil-helper” failed (this candidate is no longer pending — approved or dismissed elsewhere).'),
        ).not.toBeInTheDocument(),
      )
    } finally {
      confirmSpy.mockRestore()
  ...
  it('a not-found approve refusal survives the refetch that removes its row', async () => {
    ...
    expect(
      screen.getByText('Approval of “auto/evil-helper” failed: this candidate is no longer pending — it was approved or dismissed elsewhere.'),
    ).toBeInTheDocument()
  })
  ...
  it('the panel notice is evicted when its subject slug is restaged', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText('Approval of “auto/evil-helper” failed: this candidate is no longer pending — it was approved or dismissed elsewhere.'),
      ).not.toBeInTheDocument(),
    )
  })
})
