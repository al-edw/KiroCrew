...
class TestValidatePermissions:
    ...
    def test_session_approval_warning(self):
        m = AppManifest(
            name="test-app",
            version="1.0.0",
            displayName="Test",
        ...
        result = validate_permissions(m)
        assert any("session approval" in w.lower() for w in result.warnings)
...
class TestSessionApprovalPermission:
    def test_enabled_app_with_grant_is_allowed(self, monkeypatch):
        ...
        monkeypatch.setattr("kiro_crew.apps.manager.is_app_enabled", lambda name: name == "test")
