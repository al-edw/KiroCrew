...
class TestCommandsDiverged:
    ...
    @pytest.mark.skipif(
        platform_compat.IS_WINDOWS,
        reason="POSIX-only: symlink creation needs no elevation there.",
    )
    def test_symlink_and_target_do_not_diverge(self, tmp_path) -> None:
        """A stable launcher path and its versioned target are one binary.

        Their basenames differ, so only resolving both tells this apart from a
        genuinely changed command.
        """
        from kiro_crew.mcp_discovery import _commands_diverged

        target = _make_executable(tmp_path / "1.0.1" / "srv-1.0.1")
        link = tmp_path / "srv"
        link.symlink_to(target)
        assert _commands_diverged(str(link), str(target)) is False
