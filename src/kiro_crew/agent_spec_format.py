...
class _FrontmatterLoader(yaml.SafeLoader):  # type: ignore[misc]
    ...
    * **Aliases** (``&anchor`` / ``*anchor``) are refused at compose time. An
      alias is a shared reference, and a shared reference is what makes a
      document a graph: a self-reference recurses forever, and a chain of
