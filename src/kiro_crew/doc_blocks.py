...
def _sdt_children(parent: Any, tag: str = "") -> Iterator[Any]:
    """Children of *parent*, descending through ``w:sdt`` content controls.

    A template-produced document wraps things in a structured document tag --
    whole body sections, a repeating row, a single cell -- so what the walker
    wants is not a direct child of where it looks, and reading only direct
    children silently drops everything inside. One rule for every level,
    because each level that had its own copy missed the one the copy was not
    written for.

    *tag* narrows the yield to that element; empty yields every non-``sdt``
...
def _docx_para_runs(para: Any, budget: _Budget) -> list[Block]:
    ...
    merged runs become -- after its runs are gathered. Word splits a sentence at
    every formatting change, so a credential that crosses a bold boundary arrives
    as two runs; charging run by run could keep the first and refuse the second,
    and the redactor -- which matches whole tokens on the joined paragraph --
    would then see a prefix its patterns do not match. The unit of refusal has to
    be the unit the redactor reads, so a paragraph that does not fit is dropped
    whole.
    """
    runs: list[Block] = []
    ...
    if not budget.spend("".join(run["text"] for run in merged), len(merged)):
        return []
    return merged
