...
async def _authorize_crew_read(request: web.Request, operation: str) -> "web.Response | None":
    ...
    **Three ways a non-owner can reach an internal-secret route**, each refused
    positively rather than by a single ``if request.get("app")`` denylist — the
    shape that let the second one through:
    ...
    2. *Channel-minted.* A channel-born session carries NO app claim —
       ``derive_caller_app`` returns ``""`` for it, because "a channel/cron-born
       slot is created without one". So a Slack or Discord participant's session
       arrived indistinguishable from the owner under an app-only check. It is
       refused here on the calling session's own namespace.
    ...
    The channel test reads ``X-Session-Key``, which is server-set and attested:
    ...
    # Deferred, like _cap_str's relay import and for a second reason of its own:
    # this module is on the gateway's boot path, and importing a NAME from
    # messaging.link executes messaging/__init__ first, which pulls driver -> acp
    # -> hooks. That module's own docstring asks readers outside `messaging` not to
    # do that at import time. A crew read is the only caller here, so the cost is
    # paid on the call instead of on every boot.
    from kiro_crew.messaging.link import is_channel_session_key

    if is_channel_session_key(request.headers.get("X-Session-Key", "")):
        _audit(operation, "denied", error="channel-minted identity rejected")
        return web.json_response({"error": "forbidden", "code": "owner_only"}, status=403)
