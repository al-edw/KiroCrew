...
def _orphaned_queued_rows(store: TaskStore, wanted: frozenset[str]) -> list[TaskRecord]:
    ...
    for kind in sorted(wanted):
        offset = 0
        while True:
            page = store.list_rows(state=QUEUED, kind=kind, limit=_ADOPT_SCAN_PAGE, offset=offset)
            rows.extend(
                rec
                for rec in page
            ...
            if len(page) < _ADOPT_SCAN_PAGE:
                break
            # Paging by OFFSET over a state this sweep does not write: the caller
            # settles the rows it is handed after every kind is read, so no row
            # leaves ``queued`` under the cursor and none is skipped.
