...
class DependencyCoordinator:
    ...
    def _wake_through_or_requeue(self, task_id: str, generation: int | None, scope: str) -> bool:
        ...
        self._requeue_waiter(task_id, generation, scope)
        return False

    def _requeue_waiter(self, task_id: str, generation: int | None, scope: str) -> None:
        ...
        with self._lock:
            ...
            # Due on the next tick rather than instantly: the seam refused for a
            # reason that is unlikely to have changed within one pass.
            sched.retry_at = max(sched.retry_at, self.now() + self._wake_spacing)
        logger.info(
            "dependency coordinator: %s refused its delegated wake; requeued on %s",
            task_id,
            scope,
        )
