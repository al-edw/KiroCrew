...
proof's wall time, and reaps the process it started and, on POSIX, its process group, so a helper a
proof stood up and never stopped does not outlive the verdict (Windows kills the direct child only).
It does not attempt kernel-level isolation.
